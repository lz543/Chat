...
//将图片路径转化为二进制数组
QByteArray Server::imageToByteArray(const QString &imagePath,int quality)
{
...
//处理登录
void Server::processLogin(QTcpSocket* clientSocket, const QJsonObject& message){
...
//处理消息
void Server::processMessage( const QJsonObject& message){
...
//处理注册
void Server::processEnroll(QTcpSocket* clientSocket, const QJsonObject& message){
...
//处理好友列表
void Server::processFriends(QTcpSocket* clientSocket, const QJsonObject& message){
...
//处理查找
void Server::processSearch(QTcpSocket* clientSocket, const QJsonObject& message){
...
void Server::processAdd(const QJsonObject& message){
...
//处理获取不在线时的离线消息
void Server::processRmessage(QTcpSocket* clientSocket, const QJsonObject& message){
    int userId = message["userId"].toInt();
...
//处理客户端对添加的回应
void Server::processAddResponse(QTcpSocket* clientSocket, const QJsonObject& message){
